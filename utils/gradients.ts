...
export const generateSquareSVG = (s: string, size = 256): string => {
  const [c1, c2] = generateColours(s);

  const svg = `
<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" fill="none" xmlns="http://www.w3.org/2000/svg">
  <rect height="${size}" width="${size}"  fill="url(#gradient)" />
  <defs>
    <linearGradient id="gradient" x1="0" y1="0" x2="${size}" y2="${size}" gradientUnits="userSpaceOnUse">
      <stop stop-color="${c1}" />
      <stop offset="1" stop-color="${c2}" />
...
export const generateBase64SVG = (
  s: string,
  size = 256,
  shape = "circle"
): string => {
  const svg =
    shape === "circle" ? generateSVG(s, size) : generateSquareSVG(s, size);
